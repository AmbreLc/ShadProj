...
#define NUM_OCTAVES 5
	...
	fixed4 frag(vertexOutput input) : COLOR
	{
		...
		// UV Debug
		//return float4(input.uv.x, _input.uv.y, 0, 1);
		...
		if (//input.uv.y < 1 - input.uv.x + _Border && input.uv.y > 1 - input.uv.x - _Border)
			(input.uv.y < _TestUV + _Border && input.uv.y > _TestUV - _Border)
			 || (input.uv.x < _TestUV + _Border && input.uv.x > _TestUV - _Border))
			//|| (input.uv.z < _TestUV + _Border && input.uv.z > _TestUV - _Border))
